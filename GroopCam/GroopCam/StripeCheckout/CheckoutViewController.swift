...
//    var backendBaseURL: String? = "https://groopcammanual.herokuapp.com/"
    var backendBaseURL: String? = "https://groopcammanual4.herokuapp.com/"
    

//    var backendBaseURL: String? = "https://example-mobile-backend-master.herokuapp.com/"
    ...
    init(products: [QuantityObject], settings: Settings, totalPrice: String, descFinal: String) {
...
//        self.hasDiscount = false
        ...
        let totalF = totalP.components(separatedBy: "$")[1]
        let price = Double(totalF)
        let amount = Double(price! * 100)
...
extension CheckoutViewController: STPPaymentContextDelegate {
    ...
    func paymentContext(_ paymentContext: STPPaymentContext, didCreatePaymentResult paymentResult: STPPaymentResult, completion: @escaping STPPaymentStatusBlock) {
        ...
        MyAPIClient.sharedClient.createPaymentIntent(products: self.products, shippingMethod: paymentContext.selectedShippingMethod, shippingAddress: paymentContext.shippingAddress, amount: self.paymentContext.paymentAmount, paymentResult: paymentResult, descFinal: self.desc) { result in
            switch result {
...
//                        self.navigationController?.popToRootViewController(animated: true)
    ...
    func paymentContext(_ paymentContext: STPPaymentContext, didUpdateShippingAddress address: STPAddress, completion: @escaping STPShippingMethodsCompletionBlock) {
        ...
        upsGround.amount = 1.50
        upsGround.label = "UPS Ground"
        upsGround.detail = "Arrives in 3-5 days"
        upsGround.identifier = "ups_ground"
        let upsWorldwide = PKShippingMethod()
        upsWorldwide.amount = 1.50
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            if address.country == nil || address.country == "US" {
                ...
                completion(.valid, nil, [upsWorldwide], upsWorldwide)
            }
        }
    }
